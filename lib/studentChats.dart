...
Future<List<chatquerylistmodal>> chatqueryfunction() async {
  ...
  final response = await http.get(Uri.parse('http://shikshaappservice.outomate.com/ShikshaAppService.svc/GetChatTopicqryshow/TopicFor/Stu/brid/'+BranchID+'/CreatedbyId/'+studentid));
  print('http://shikshaappservice.outomate.com/ShikshaAppService.svc/GetChatTopicqryshow/TopicFor/Stu/brid/'+BranchID+'/CreatedbyId/'+studentid);
  if (response.statusCode == 200) {
    final parsed = json.decode(response.body).cast<Map<String, dynamic>>();
    print("sssssssssss${parsed[0]['ChatTopicId_Fk']}");
    return parsed.map<chatquerylistmodal>((json) => chatquerylistmodal.fromMap(json)).toList();
  } else {
...
class studentChats extends StatefulWidget {
  const studentChats({Key? key}) : super(key: key);
...
class _studentChatsState extends State<studentChats> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title:Text('Chats Query',style:TextStyle(fontWeight: FontWeight.bold,color:appcolors.whiteColor),),
        ),
        body: Container(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: (){
          Navigator.of(context).push(MaterialPageRoute(builder: (context) => createquerypage()));
        },
        tooltip: 'Increment',
        ...
        child: ListTile(
          ...
          leading: CircleAvatar(
            ...
            child: CircleAvatar(
              ...
              backgroundImage: NetworkImage('https://cdn-icons-png.flaticon.com/512/6728/6728469.png',),
            ),
          ),
          title: Center(
            child: Row(
              ...
              children: [
                Center(
                  child: Column(
                    ...
                    children: [
                      Container(height: 20,width: 200,child: Text("Query No : ${snapshot.data![index].QryNo}",style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 16),overflow: TextOverflow.ellipsis,)),
                      Container(height: 20,width: 200,child: Text("Topic Name : ${snapshot.data![index].ChatTopicName}",style: const TextStyle(fontWeight: FontWeight.bold,fontSize: 12),overflow: TextOverflow.ellipsis,)),
